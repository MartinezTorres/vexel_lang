...
std::string CodeGenerator::gen_type(TypePtr type) {
    ...
    switch (type->kind) {
        case Type::Kind::Primitive: {
            switch (type->primitive) {
                ...
                case PrimitiveType::FixedUInt: {
                    ...
                    if (total_bits_i64 <= 0) {
                        throw CompileError("Invalid fixed-point storage width in C backend", type->location);
                    }
        ...
        case Type::Kind::Matrix:
            throw CompileError("Internal error: vector/matrix type reached C backend after frontend lowering",
                               type->location);
        ...
        case Type::Kind::TypeOf:
            throw CompileError("Internal error: unresolved #[...] reached C backend", type->location);
    }
...
void CodeGenerator::release_temp(const std::string& temp) {
    (void)temp;
    // Temp names are currently type-erased. Reusing names across scalar/struct temps can emit
    // invalid reassignments (e.g. `tmp = 0` after `tmp` was declared as an arbitrary-width struct).
    // Prefer correctness over reuse; Step 8 can reintroduce typed reuse if needed.
}
...
std::string CodeGenerator::ensure_comparator(TypePtr type) {
    ...
    switch (type->kind) {
        ...
        case Type::Kind::Vector:
        case Type::Kind::Matrix:
            throw CompileError("Internal error: vector/matrix type reached C backend comparator generation after frontend lowering",
                               type->location);
        case Type::Kind::Named: {
