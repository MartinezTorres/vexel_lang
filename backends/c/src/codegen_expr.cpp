...
std::string fixed_muldiv_raw_expr_codegen(const vexel::TypePtr& fixed_type,
    ...
    throw vexel::CompileError("Unsupported fixed-point operator in C codegen: " + op, loc);
}
...
std::string CodeGenerator::gen_range(ExprPtr expr) {
    ...
    uint64_t elem_bits = 0;
    if (elem_extint) {
        analyze_extint_type(expr->type->element_type, elem_signed, elem_bits);
    }
    if (start < end) {
