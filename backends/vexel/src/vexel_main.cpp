...
// Dedicated CLI for the lowered-Vexel backend.
static void print_usage(const char* prog) {
    std::cout << "Vexel Compiler (Vexel backend)\n";
    std::cout << "Usage: " << prog << " [options] <input.vx>\n\n";
    std::cout << "Options:\n";
    std::cout << "  -o <path>    Output path (base name for generated files, default: out)\n";
    std::cout << "  -b <name>    Backend (optional compatibility flag: accepts vexel only)\n";
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (vexel::try_parse_common_compiler_option(argc, argv, i, opts, parse_error)) {
            ...
            if (backend_name != "vexel") {
                std::cerr << "Error: vexel CLI only supports backend=vexel\n";
                return 1;
            }
        } else if (argv[i][0] == '-') {
