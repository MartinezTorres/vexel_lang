...
if ! rg -q "std::queue<size_t> expr_queue_" "$OPT_CPP"; then
  echo "optimizer must keep a targeted expression fallback queue for unresolved root nodes" >&2
  exit 1
fi
...
if rg -q "enqueue_expr\\(i\\);" "$OPT_CPP"; then
  echo "optimizer must not eagerly enqueue all expression nodes up front" >&2
  exit 1
fi
