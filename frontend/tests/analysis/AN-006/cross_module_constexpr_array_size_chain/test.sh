...
if ! rg -q "\\b36\\b" out.vx; then
  echo "cross-module constexpr array size chain did not fold to expected value"
  exit 1
fi

if rg -q "dims::" out.vx; then
  echo "cross-module constexpr dependency unexpectedly survived lowering"
  exit 1
fi
