...
CCodegenResult CodeGenerator::generate(const Module& mod, TypeChecker* tc) {
    ...
    emit_header("// Dead code elimination: only functions reachable from exports are generated");
    emit_header("");
...
void CodeGenerator::gen_func_decl(StmtPtr stmt) {
    ...
    bool body_has_calls = false;
    if (stmt->body) {
        std::unordered_set<std::string> body_calls;
        collect_calls(stmt->body, body_calls);
        body_has_calls = !body_calls.empty();
    }

    if (stmt->body) {
        if (type_checker && stmt->params.empty() && stmt->ref_params.empty() && !body_has_calls) {
            CompileTimeEvaluator evaluator(type_checker);
            CTValue result;
        ...
        if (!handled_body && type_checker && stmt->body->kind == Expr::Kind::Block && !body_has_calls) {
