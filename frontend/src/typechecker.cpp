...
bool type_is_concrete(TypeChecker* tc, TypePtr type) {
    if (!type) return false;
    if (tc) {
        type = tc->resolve_type(type);
    }
    switch (type->kind) {
        case Type::Kind::TypeVar:
            return false;
        case Type::Kind::Array:
            return type_is_concrete(tc, type->element_type);
        default:
            return true;
    }
}
...
struct TypeUseValidator {
    ...
    void validate_expr(ExprPtr expr, bool value_required) {
        if (!expr) return;
        if (value_required && !expr->is_expr_param_ref && !type_is_concrete(checker, expr->type)) {
    ...
    void validate_stmt(StmtPtr stmt) {
        ...
        switch (stmt->kind) {
            case Stmt::Kind::VarDecl:
                if (!type_is_concrete(checker, stmt->var_type)) {
...
void TypeChecker::validate_type_usage(const Module& mod, const AnalysisFacts& facts) {
    ...
    for (const auto& stmt : mod.top_level) {
        ...
        for (const auto& field : stmt->fields) {
            if (!type_is_concrete(this, field.type)) {
    ...
    for (const auto& pair : functions) {
        ...
        for (const auto& param : func->params) {
            ...
            if (!type_is_concrete(this, param.type)) {
        ...
        for (size_t i = 0; i < func->ref_params.size(); ++i) {
            ...
            if (!type_is_concrete(this, ref_type)) {
        ...
        if (!func->return_types.empty()) {
            ...
            if (!type_is_concrete(this, func->return_type)) {
    ...
    for (const auto& stmt : mod.top_level) {
        ...
        if (!type_is_concrete(this, stmt->var_type)) {
