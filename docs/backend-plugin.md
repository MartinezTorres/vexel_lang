...
This document captures the current plugin method for Vexel backends. The goal is a small, stable API that lets backends live in-tree or out-of-tree while keeping the compiler core simple. For step-by-step instructions, see `docs/backend-authoring.md`.
